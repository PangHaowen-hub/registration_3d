...
def volgen(
    ...
    if isinstance(vol_names1, str):
        if os.path.isdir(vol_names1):
            vol_names1 = os.path.join(vol_names1, '*')
        vol_names1 = glob.glob(vol_names1)

    if isinstance(segs, list) and len(segs) != len(vol_names1):
        raise ValueError('Number of image files must match number of seg files.')

    while True:
        # generate [batchsize] random image indices
        indices = np.random.randint(len(vol_names1), size=batch_size)
        ...
        imgs1 = [utils.load_volfile(vol_names1[i], **load_params) for i in indices]
        imgs2 = [utils.load_volfile(vol_names2[i], **load_params) for i in indices]
...
def scan_to_scan(vol_names1, vol_names2, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    """
    ...
    """
    zeros = None
    gen = volgen(vol_names1, vol_names2, batch_size=batch_size, **kwargs)
    while True:
        temp = next(gen)
        scan1 = temp[0][0]
        scan2 = temp[1][0]
