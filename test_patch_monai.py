...
from voxelmorph.utils import default_unet_features
from voxelmorph import layers
from voxelmorph.modelio import LoadableModel, store_config_args
import os
import torch
import tqdm
import numpy as np
...
class VxmDense(LoadableModel):
    ...
    def forward(self, x):
        ...
        '''
        source = torch.unsqueeze(x[:, 0, :, :, :], dim=1)
        target = torch.unsqueeze(x[:, 1, :, :, :], dim=1)
        ...
        output = torch.cat([y_source, pos_flow], dim=1)
...
if __name__ == '__main__':
    ...
    for i in tqdm.trange(len(fixed_list)):
        ...
        inferer = SlidingWindowInferer(roi_size=(256, 256, 256), sw_batch_size=1, overlap=0.5,
                                       mode='gaussian')
        with torch.no_grad():
            pred = inferer(inputs=x, network=model)
        pred = pred.detach().cpu().numpy().squeeze()
        _, fullflname = os.path.split(moving_list[i])
        moved_img = sitk.GetImageFromArray(np.squeeze(pred[0, :, :, :]))
        ...
        warp_img = sitk.GetImageFromArray(np.squeeze(pred[1:, :, :, :]))
